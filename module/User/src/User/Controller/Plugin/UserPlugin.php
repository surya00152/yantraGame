...
//    }
            ...
            if ($userData['userRoll'] == 'local') {
                ...
                if ($totalPrice <= $userData['avaiPurchaseBal']){
                  try {
                        ...
                        $response['yantra'] = $this->getAllYantraForUser($userData);
                        $response['status'] = 'success';
                        $response['message'] = 'Bet submited successfully.';
    ...
    public function getDrowStatus($post)
    {
        ...
        error_log($log,3,'drowstatus.log');
        return $response;
    }
                ...
                if(isset($post['date'])) {
                    ...
                    if(count($ticketData) > 0) {
                        foreach($ticketData as $ticket) {
                            ...
                            $reportArray[$key][$ticket['yantraId']] = $ticket['quantity'];                 
                        }
                        //get total
            ...
            if ($userData['userRoll'] == 'local') {
                ...
                if(count($daywiseReport) > 0) {
                    foreach ($daywiseReport as $rKey => $report) {
                        ...
                        if (empty($report['creditBal'])) {
                            $daywiseReport[$rKey]['creditBal'] = 0;
                        }
                        if (empty($report['debitBal'])) {
                            $daywiseReport[$rKey]['debitBal'] = 0;
                        }
                    }
                }                
                ...
                if ($localUserData['avaiPurchaseBal'] >= $localUserData['balReq']) {
                    ...
                    try {
                        ...
                        $notificationData = array (
                            ...
                            'reqTo' => $localUserData['Id'],
                            'requestedName' => $userData['name'],
                            'message' => 'Your balance will be transfer by Agent. Transfer debit is: '.$localUserData['balReq'],
                            'date' => $this->getAppService()->getDateTime()
                        );                            
                        //set notification message
                        $this->getNotificationModel()->createNotification($notificationData);
    ...
    public function getNotification($post)
    {
        ...
        if (count($userData)) {
            try {
                $response['status'] = 'success';
                $notifications = $this->getNotificationModel()->getNotification($userData['Id']);
                
                if (count($notifications) > 0) {
                    foreach ($notifications as $nKey => $note) {
                        $notifications[$nKey]['date'] = date('d-m-Y h:i a', strtotime($note['date']));
                    }
                }
    ...
    public function transactionReport($post,$isAdmin=false)
    {
        $response = array();
        if ($isAdmin == true && isset($post['Id'])) {
            $userData = $post['userData'];
        ...
        if (count($userData)) {
            try {
                $response['status'] = 'success';
                $transaction = $this->getTransactionModel()->getTransactionReport($userData['Id'],$post['date']);
                if (count($transaction) > 0) {
                    foreach ($transaction as $nKey => $tran) {
                        $transaction[$nKey]['time'] = date('h:i a', strtotime($tran['time']));
                    }
                }
